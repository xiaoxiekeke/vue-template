	...
	<div class="consump__detail__call">
		...
		<div class="cdc__table" v-loading="tableLoading">
			...
			<el-table @sort-change="sortChange" :data="tableData" :border="true" stripe >
              ...
              {{ scope.row.callType==1 ? scope.row.ani:(scope.row.callType==2 ? scope.row.dnis:'--') }}
            </template>
			    </el-table-column>
			    <el-table-column
			    	align="center"
              ...
              {{ scope.row.itemDesc ? scope.row.itemDesc:'--' }}
			    ...
			    <el-table-column
		      	...
		      	label="线路">
		      	<template slot-scope="scope">
              {{ scope.row.callType==1 ? scope.row.dnis:(scope.row.callType==2 ? scope.row.ani:'--') }}
            </template>
			    </el-table-column>
			    <el-table-column
			    	align="center"
		...
		methods : {
			...
			reloadTable : function(){
				...
				let param = {
					...
					// callTime : this.condition.callTime,
