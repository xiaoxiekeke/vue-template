	 ...
	 <div class="report" v-loading="loading">
        ...
        <div class="report__item report__overview">
	        ...
	        <div class="report__data">
	            <div class="report__data__content">
		            <div class="report__desc">
		                <p class="report__lbl">号码量</p>
		                <span class="report__value">{{summary.totalTel}}</span>个
		            </div>
	            </div>
	        </div>
	        <div class="report__data">
		...
		<div class="report__item report__callAnalysis">
			<div class="callDurationOut pie"></div>
			<div class="interactCountOut pie"></div>
	   	</div>
        
        
        <!-- <h3 class="report__head">通话量趋势</h3> -->
        <div v-if="chartIsNull" class="report__item report__chartOut">
        	<span class="empty">暂无数据</span>
        </div>
        <div v-show="!chartIsNull" class="report__item report__chartOut"></div>
...
export default {
	...
	methods : {
	    ...
	    loadCallDurationChart:function(){//通话时长饼图
	    	...
	    	this.CallDurationChart = echarts.init(document.querySelector('.callDurationOut'));
	    	var option = {
	    ...
	    loadInteractCountChart:function(){//对话轮次饼图
	    	...
	    	this.InteractCountChart = echarts.init(document.querySelector('.interactCountOut'));
	    	var option = {
	    ...
	    loadCallChart : function(){
	    	this.loading = false;
	    	var callAnswerData = this.completeDim(this.callAnalysis.callAnswer[this.returnType]);
			var callAnswerList = []
			for(var i in callAnswerData){
				callAnswerList.push({
					connectNum: callAnswerData[i],
					time : i
				});
			}
			callAnswerList.sort(function(a,b){return parseInt(a.time) - parseInt(b.time)})
			callAnswerData=this.transferToChatData(callAnswerList)

			var callAnswerRate = this.completeDim(this.callAnalysis.callAnswerRate[this.returnType]);
			var callAnswerRateList = []
			for(var i in callAnswerRate){
				callAnswerRateList.push({
					connectNum: callAnswerRate[i],
					time : i
				});
			}
			callAnswerRateList.sort(function(a,b){return parseInt(a.time) - parseInt(b.time)})
			callAnswerRate=this.transferToChatData(callAnswerRateList)
	        ...
	        var option = {
			    ...
			    yAxis: [
			        {
			            name: '',
			            min:0,
			            max: callAnswerData.series.max()?Math.ceil(callAnswerData.series.max()/5)*5:100,
			            type: 'value',
			            splitNumber:5,
			            interval:callAnswerData.series.max()?Math.ceil(callAnswerData.series.max()/5):20,
			            axisLine:{
			            	lineStyle:{
			            		color:'#999'
			            	}
			    ...
			    series: [
			        ...
			        },
			        {
			            ...
			            areaStyle: {
			            	color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
			                	{ offset: 0, color: "rgba(0,183,146, 1)"},
			                    { offset: 1, color: "rgba(0,183,146, 0)"}
			                ])
			            },
			            lineStyle: {
			                width: 1
			            },
			            data:callAnswerRate.series
			        }
			    ]
			};
	       	this.myChart = echarts.init(document.querySelector('.report__chartOut'));
